...
import FundamentalNotesTable from "./components/FundamentalNotesTable";
import ChromaticIntervalsTable from "./components/ChromaticIntervalsTable";
import {
  PlayArrow,
  Pause,
  RestartAlt,
...
// Base URL de las muestras (parametrizable por env var VITE_TONE_SAMPLES_BASE_URL)
const TONE_SAMPLES_BASE_URL =
  typeof import.meta !== "undefined" &&
  import.meta.env &&
  import.meta.env.VITE_TONE_SAMPLES_BASE_URL
    ? import.meta.env.VITE_TONE_SAMPLES_BASE_URL
...
function labelSPN(key) {
  // "c/4" -> "C4"
  const [s, o] = key.split("/");
  return `${LETTERS[s]}${o}`;
}
function labelSolfege(key) {
  // "c/4" -> "Do4"
  const [s, o] = key.split("/");
  return `${SOLFEGE[s]}${o}`;
}
...
export default function App() {
  ...
  const [status, setStatus] = useState(
    "Listo: genera un dictado y coloca tus notas en el pentagrama inferior.",
  );
  ...
  const ledgerSet = useMemo(
    () => new Set(currentClefConfig.ledgerLineNotes),
    [selectedClef],
  );
  ...
  useEffect(() => {
    drawDictation();
  }, [dictation, dur, mode, currentPlayingNote, selectedClef]);
  useEffect(() => {
    drawUser();
  }, [userNotes, dur, mask, selectedClef]);
  ...
  useEffect(() => {
    if (sequentialMode === "both") {
      setNum(15); // 8 ascendente + 7 descendente
    } else if (
      sequentialMode === "ascending" ||
      sequentialMode === "descending"
    ) {
      setNum(8);
    }
  }, [sequentialMode]);
  ...
  function drawDictation() {
    ...
    vfA.current = new Factory({
      renderer: { elementId: wrapA.current.id, width: 1100, height: 200 },
    });
    ...
    for (let i = 0; i < N; i++) {
      ...
      const noteX =
        startX + currentBeat * beatWidth + (noteBeatDuration * beatWidth) / 2;
      ...
      notePositions.push({
        x: noteX,
        beat: currentBeat,
        endBeat: currentBeat + noteBeatDuration,
      });
    ...
    for (let i = 0; i < notePositions.length - 1; i++) {
      ...
      if (currentCompasBeats >= 4) {
        const currentNoteEnd =
          notePositions[i].x + (noteBeatDuration * beatWidth) / 2;
        const nextNoteStart =
          notePositions[i + 1].x - (noteBeatDuration * beatWidth) / 2;
  ...
  function drawUser() {
    ...
    vfB.current = new Factory({
      renderer: { elementId: wrapB.current.id, width: 1100, height: 200 },
    });
  ...
  function onClickStaff(e) {
    ...
    setUserNotes((prev) => {
      const next = [...prev];
      if (activeIdx < next.length) next[activeIdx] = k;
      return next;
    });
    setUserInputs((prev) => {
      const next = [...prev];
      if (activeIdx < next.length) next[activeIdx] = label;
      return next;
    });
    setActiveIdx((i) => {
      const len = dictation.length;
      for (let j = i + 1; j < len; j++) {
        if (!userNotes[j]) return j;
      }
      for (let j = 0; j < len; j++) {
        if (!userNotes[j]) return j;
      }
  ...
  function newDictation() {
    ...
    if (sequentialMode === "random") {
      if (n === 1) {
        setDictation([
          currentStartNote === currentEndNote
            ? currentStartNote
            : Math.random() < 0.5
              ? currentStartNote
              : currentEndNote,
    ...
    const modeLabel =
      {
        ...
        both: "secuencial (ascendente o descendente)",
      }[sequentialMode] || "aleatorio";

    setStatus(
      `Nuevo dictado de ${n} notas (${modeLabel}). Coloca tu respuesta y califica.`,
    );
  }
  ...
  function grade() {
    ...
    if (mode === "exam") {
      const m = userNotes.map((k, i) => k && k === dictation[i]);
      ...
      if (answered === dictation.length) {
        setStatus(
          `Resultado final: ${correct}/${dictation.length} (${Math.round((100 * correct) / dictation.length)}%).`,
        );
      } else {
        setStatus(
          `Progreso: ${correct}/${answered} correctas de ${answered}/${dictation.length} respondidas (${pct}% acierto).`,
        );
      }
      return;
    }
    if (userNotes.some((v) => !v)) {
      const done = userNotes.filter(Boolean).length;
      setStatus(`Faltan notas: ${done}/${dictation.length}.`);
      return;
    }
    const m = userNotes.map((k, i) => k === dictation[i]);
    const ok = m.filter(Boolean).length;
    const pct = Math.round((100 * ok) / dictation.length);
    setMask(m);
    setStatus(`Resultado: ${ok}/${dictation.length} (${pct}%).`);
  }
    ...
    <Container className="wrap">
      <Stack spacing={2}>
        <Typography variant="h5" sx={{ fontWeight: 700, color: "#0b2a50" }}>
          🎧 Entrenador de dictados melódicos — Clave de{" "}
            ...
            <Grid item xs={12} md={12}>
              ...
              >
                ...
                <Button
                  variant="contained"
                  color="primary"
                  onClick={() => playSequence(dictation)}
                  startIcon={isPlaying ? <Pause /> : <PlayArrow />}
                >
                  {isPlaying ? "Pausar" : "Reproducir"}
                </Button>
                <Button variant="outlined" onClick={startMet}>
                  Metrónomo
                </Button>
              </Stack>
            </Grid>
        ...
        <Paper className="staff">
          ...
          {mode === "train" &&
            (currentNoteDisplay.show ||
              (isPlaying && currentNoteDisplay.spn)) && (
              <Box
                sx={{
                  display: "flex",
              ...
              {[0, 1, 2, 3].map((beat) => (
                <Box
                  ...
                  sx={{
                    ...
                    borderRadius: "50%",
                    backgroundColor:
                      metronomeBeat === beat ? "#2196f3" : "#e0e0e0",
                ...
                >
                  <Typography
                    variant="caption"
                    sx={{
                      color: metronomeBeat === beat ? "white" : "#666",
                      fontWeight: "bold",
                    }}
                  >
                    {beat + 1}
          ...
          {mode === "train" && (
            <Box sx={{ mt: 1, display: "flex", gap: 1, flexWrap: "wrap" }}>
              {dictation.map((k, idx) => (
                <Chip
                  key={`L-${idx}`}
                  label={`${labelSPN(k)} (${labelSolfege(k)})`}
                  size="small"
                />
              ))}
            </Box>
        ...
        <Paper className="staff">
          <div className="legend">
            Pentagrama 2 (tu respuesta) – clic para añadir · clic derecho o ⌫
            para deshacer
          </div>
            ...
            {Array.from({ length: dictation.length }, (_, i) => {
              ...
              const helper = txt
                ? p.valid
                  ...
                  : "Ej: C4 o Do4"
                : "Ej: C4 o Do4";
              const color = mask ? (mask[i] ? "success" : "error") : undefined;
              return (
        ...
        >
          ...
          <Stack direction="row" spacing={1}>
            <Button
              ...
              onClick={() => {
                setUserNotes(Array(dictation.length).fill(null));
                setUserInputs(Array(dictation.length).fill(""));
                setActiveIdx(0);
                setMask(null);
                setStatus("Respuesta borrada.");
              }}
            >
              Borrar respuesta
            </Button>
