...
const ARPEGGIO_MAJOR_BASS: Record<RootNote, readonly string[]> = {
  ...
  Db: ["db/2", "f/2", "ab/2", "db/3", "db/3", "ab/2", "f/2", "db/2"],
  D: ["d/2", "f#/2", "a/2", "d/3", "d/3", "a/2", "f#/2", "d/2"],
  Eb: ["eb/2", "g/2", "bb/2", "eb/3", "eb/3", "bb/2", "g/2", "eb/2"],
  E: ["e/2", "g#/2", "b/2", "e/3", "e/3", "b/2", "g#/2", "e/2"],
  F: ["f/2", "a/2", "c/3", "f/3", "f/3", "c/3", "a/2", "f/2"],
  Gb: ["gb/2", "bb/2", "db/3", "gb/3", "gb/3", "db/3", "bb/2", "gb/2"],
  G: ["g/2", "b/2", "d/3", "g/3", "g/3", "d/3", "b/2", "g/2"],
  Ab: ["ab/2", "c/3", "eb/3", "ab/3", "ab/3", "eb/3", "c/3", "ab/2"],
  A: ["a/2", "c#/3", "e/3", "a/3", "a/3", "e/3", "c#/3", "a/2"],
  Bb: ["bb/2", "d/3", "f/3", "bb/3", "bb/3", "f/3", "d/3", "bb/2"],
  B: ["b/2", "d#/3", "f#/3", "b/3", "b/3", "f#/3", "d#/3", "b/2"],
} as const;

const ARPEGGIO_MINOR_BASS: Record<RootNote, readonly string[]> = {
  A: ["a/2", "c/3", "e/3", "a/3", "a/3", "e/3", "c/3", "a/2"],
  Ab: ["ab/2", "cb/3", "eb/3", "ab/3", "ab/3", "eb/3", "cb/3", "ab/2"],
  B: ["b/2", "d/3", "f#/3", "b/3", "b/3", "f#/3", "d/3", "b/2"],
  Bb: ["bb/2", "db/3", "f/3", "bb/3", "bb/3", "f/3", "db/3", "bb/2"],
  C: ["c/2", "eb/2", "g/2", "c/3", "c/3", "g/2", "eb/2", "c/2"],
  Db: ["db/2", "fb/2", "ab/2", "db/3", "db/3", "ab/2", "fb/2", "db/2"],
  D: ["d/2", "f/2", "a/2", "d/3", "d/3", "a/2", "f/2", "d/2"],
  Eb: ["eb/2", "gb/2", "bb/2", "eb/3", "eb/3", "bb/2", "gb/2", "eb/2"],
  E: ["e/2", "g/2", "b/2", "e/3", "e/3", "b/2", "g/2", "e/2"],
  F: ["f/2", "ab/2", "c/3", "f/3", "f/3", "c/3", "ab/2", "f/2"],
  Gb: ["gb/2", "a/2", "db/3", "gb/3", "gb/3", "db/3", "a/2", "gb/2"],
...
const FIGURE_TO_BEATS: Record<FigureValue, number> = FIGURE_OPTIONS.reduce(
  (acc, opt) => {
...
export default function BassScaleSuite() {
  ...
  const handlePatternToggle =
    (patternId: PatternId) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const selectorCard = (
    ...
    >
      ...
      <FormControl fullWidth size="small">
        ...
        <Select label="Figura" value={noteFigure} onChange={handleFigureChange}>
      ...
      />
      <FormGroup
        sx={{ border: "1px solid #e0e0e0", borderRadius: 1.5, p: 1.5 }}
      >
      ...
      <Typography variant="body2" color="text.secondary">
        Configura tonalidad, figura rítmica, BPM y qué patrones quieres
        encadenar. La suite se repetirá en bucle hasta que presiones detener.
      </Typography>
    </Paper>
  );
  ...
  const tunerCard = (
    ...
    >
      ...
      <Typography variant="body2" color="text.secondary">
        Mantén tu referencia en la misma vista. Ajusta cuerdas antes de tocar
        cada patrón.
      </Typography>
      <AlwaysOnTuner />
    </Paper>
  ...
  const guideCard = (
    ...
    >
      ...
      <Typography variant="body2" color="text.secondary">
        Elige una tonalidad, pulsa reproducir y deja que la suite te lleve por
        la escala mayor, la menor natural y ambos arpegios. Cada nota resaltada
        muestra su cifrado americano.
      </Typography>
      <Typography variant="body2" color="text.secondary">
        Usa el mismo patrón técnico para ambos arpegios: 1–3–5–8 descendiendo
        con digitación espejo para memorizar saltos amplios.
      </Typography>
    </Paper>
  );
  ...
  const orderCard = (
    ...
    >
      ...
      {hasActivePatterns ? (
        ...
        <Typography variant="body2" color="text.secondary">
          No hay patrones seleccionados. Marca al menos uno para escuchar la
          suite.
        </Typography>
      )}
    </Paper>
  ...
  const column2 = [
    patternMap["major-scale"] &&
      renderPatternCard(
        patternMap["major-scale"],
        enabledPatterns["major-scale"],
      ),
    patternMap["major-arpeggio"] &&
      renderPatternCard(
        patternMap["major-arpeggio"],
        enabledPatterns["major-arpeggio"],
      ),
    tunerCard,
  ].filter(Boolean) as React.ReactNode[];
  const column3 = [
    patternMap["minor-scale"] &&
      renderPatternCard(
        patternMap["minor-scale"],
        enabledPatterns["minor-scale"],
      ),
    patternMap["minor-arpeggio"] &&
      renderPatternCard(
        patternMap["minor-arpeggio"],
        enabledPatterns["minor-arpeggio"],
      ),
    orderCard,
    tonalitiesCard,
  ].filter(Boolean) as React.ReactNode[];
