...
export const ARPEGGIO_PATHS_MAJOR: Record<
  "treble" | "bass" | "bass_3rd",
...
export const ARPEGGIO_PATHS_MINOR: Record<
  "treble" | "bass" | "bass_3rd",
  Record<string, readonly string[]>
> = {
  treble: {
...
const INTERVAL_EXERCISES: IntervalExercise[] = [
  {
    ...
    description:
      "Lectura de segundas mayores/menores dentro del registro grave (C2–D3).",
    notes: [
      "c/2",
      "d/2",
      "e/2",
  ...
  },
  {
    ...
    description:
      "Saltos en terceras diatónicas pensados para visualización de líneas/espacios.",
    notes: [
      "c/2",
      "e/2",
      "d/2",
  ...
  },
  {
    ...
    description:
      "Patrones de cuartas y quintas para ubicar saltos frecuentes en bajo.",
    notes: [
      "c/2",
      "f/2",
      "c/2",
  ...
  },
  {
    ...
    description:
      "Saltos amplios manteniendo la referencia tonal dentro del pentagrama.",
    notes: [
      "c/2",
      "a/2",
      "d/2",
  ...
  },
  {
    ...
    description:
      "Alterna fundamental–octava para consolidar ubicación visual de las notas.",
    notes: [
      "c/2",
      "c/3",
      "d/2",
...
const SCALE_PATHS_MAJOR: Record<
  "treble" | "bass" | "bass_3rd",
  Record<string, readonly string[]>
> = {
  treble: {
...
const SCALE_PATHS_MINOR: Record<
  "treble" | "bass" | "bass_3rd",
  Record<string, readonly string[]>
> = {
  treble: {
...
function prettyFromKey(key: string) {
  const [s, o] = key.split("/");
  const symbol =
    (LETTERS as Record<string, string>)[s] ??
...
function freqOfKey(key: string) {
  return 440 * Math.pow(2, (midiFromKey(key) - 69) / 12);
}
function pianoLike(freq: number, time: number, durSec: number) {
  ...
  const PARTS = [
    ...
    { type: "sine" as OscillatorType, mult: 4, g: 0.1 },
  ];
  PARTS.forEach((p) => {
...
export default function BajoTrainer() {
  ...
  // ---- Estados base ----
  const [trainerMode, setTrainerMode] = useState<"scales" | "intervals">(
    "scales",
  );
  const [selectedClef, setSelectedClef] = useState<
    "treble" | "bass" | "bass_3rd"
  >("bass");
  const [exerciseType, setExerciseType] = useState<
    "major_scales" | "minor_scales" | "major_arpeggios" | "minor_arpeggios"
  >("major_scales");
  const [selectedKey, setSelectedKey] = useState<string>("C");
  const [selectedIntervalId, setSelectedIntervalId] = useState<string>(
    INTERVAL_EXERCISES[0]?.id ?? "interval-seconds",
  );
  ...
  const [currentExerciseDuration, setCurrentExerciseDuration] = useState<
    "w" | "h" | "q"
  >("w"); // duración actual del ejercicio completo
  ...
  const selectedInterval = (INTERVAL_EXERCISES.find(
    (ex) => ex.id === selectedIntervalId,
  ) ?? INTERVAL_EXERCISES[0]) as IntervalExercise;
  ...
  useEffect(() => {
    ...
    if (selectedInterval) {
      setCurrentScale({
        ...
        description: selectedInterval.description,
      });
    } else {
  ...
  function drawStaff() {
    ...
    const calculatedWidth =
      staveMargins + notes.length * noteWidth + LEFT_PAD + RIGHT_PAD;
    ...
    const exerciseDur = noteRotation
      ? currentExerciseDuration
      : getCurrentExerciseDuration();
  ...
  async function playScale(overrideDur?: "w" | "h" | "q") {
    ...
    const exerciseDur = noteRotation
      ? (overrideDur ?? getCurrentExerciseDuration())
      : getCurrentExerciseDuration();
    ...
    currentScale.notes.forEach((noteKey, i) => {
      ...
      Tone.Transport.schedule((time) => {
        ...
        setCurrentNoteDisplay({
          spn: prettyFromKey(noteKey),
          solfege: labelSolfege(noteKey),
          show: true,
        });
  ...
  const validKeys =
    trainerMode === "scales" ? getValidKeysForType(exerciseType) : [];
  const meta =
    trainerMode === "scales"
      ? getMetaForType(exerciseType)
      : ({} as Record<string, string>);
  const isMinorExercise =
    trainerMode === "scales" && exerciseType.includes("minor");
  const bannerBgColor =
    ...
    <Box sx={{ width: "100%", px: 2 }}>
      <Stack spacing={2}>
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} sm={6} md={2}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  value={trainerMode}
                  onChange={(e) =>
                    setTrainerMode(e.target.value as "scales" | "intervals")
                  }
                >
            ...
            <Grid item xs={6} sm={3} md={2}>
              <FormControlLabel
                control={
                  <Switch
                    checked={noteRotation}
                    onChange={(e) => setNoteRotation(e.target.checked)}
                  />
                }
            ...
            {trainerMode === "scales" ? (
                ...
                <Grid item xs={12} sm={6} md={3}>
                  <FormControl fullWidth size="small" disabled={fixedMode}>
                    ...
                    <Select
                      ...
                      value={exerciseType}
                      onChange={(e) =>
                        setExerciseType(e.target.value as typeof exerciseType)
                      }
                    >
                      <MenuItem value="major_scales">Escalas mayores</MenuItem>
                      <MenuItem value="minor_scales">Escalas menores</MenuItem>
                      <MenuItem value="major_arpeggios">
                        Arpegios mayores
                      </MenuItem>
                      <MenuItem value="minor_arpeggios">
                        Arpegios menores
                      </MenuItem>
                    </Select>
                  </FormControl>
                  ...
                  <>
                    <Grid item xs={12} sm={6} md={2}>
                      <FormControl fullWidth size="small">
                        ...
                        <Select
                          labelId="fixed-string-label"
                          label="Cuerda"
                          value={fixedString}
                          onChange={(e) =>
                            setFixedString(e.target.value as StringId)
                          }
                        >
                          {BASS_STRINGS.map((s) => (
                    ...
                    <Grid item xs={12} sm={6} md={2}>
                      <FormControl fullWidth size="small">
                        ...
                        <Select
                          labelId="fixed-fret-label"
                          label="Traste"
                          value={fixedFret}
                          onChange={(e) => setFixedFret(Number(e.target.value))}
                        >
                          {Array.from({ length: 13 }, (_, i) => i).map((fr) => (
                    ...
                    <Grid item xs={12} sm={12} md={3}>
                      <TextField
                        fullWidth
                        size="small"
                        label="Nota seleccionada"
                        value={`${prettyFromKey(fixedKey)}  ·  ${fixedSpn}`}
              ...
              <>
                <Grid item xs={12} sm={6} md={3}>
                  <FormControl fullWidth size="small">
                    <InputLabel id="interval-exercise-label">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        setSelectedIntervalId(e.target.value as string)
                      }
                    >
        ...
        <Paper sx={{ p: 2 }}>
          ...
          >
            Tip: para graves (E≈41 Hz / B≈31 Hz) una interfaz de audio rastrea
            más estable que el mic del laptop.
          </Typography>
        </Paper>
        ...
        <Paper sx={{ p: 2 }}>
            ...
            >
              ...
              >
                ...
                {noteRotation && (
                  <Typography
                    component="span"
                    sx={{ ml: 2, fontSize: "0.9em", fontWeight: "normal" }}
                  >
                    📐 Rotación por ejercicio:{" "}
                    {getExerciseDurationLabel(currentExerciseDuration)}
                  </Typography>
                )}
              </Typography>
            </Box>
            ...
            >
              ...
              >
                ...
                {noteRotation && (
                  <Typography
                    component="span"
                    sx={{ ml: 2, fontSize: "0.8em", fontWeight: "normal" }}
                  >
                    [Ejercicio:{" "}
                    {getExerciseDurationLabel(currentExerciseDuration)}]
                  </Typography>
          ...
          {currentScale && !fixedMode && (
            <Paper
              sx={{ p: 2, mt: 2, backgroundColor: "rgba(33, 150, 243, 0.03)" }}
            >
              <Typography
                variant="body2"
              ...
              >
                {currentScale.notes.map((note, idx) => {
                  const isAscending =
                    trainerMode === "scales" &&
                    idx < currentScale.notes.length / 2;
                    ...
                    >
                      ...
                      {idx < currentScale.notes.length - 1 && (
                        ...
                        >
                          {trainerMode === "scales"
                            ? isAscending &&
                              idx === currentScale.notes.length / 2 - 1
                              ? "🔄"
                              : "→"
                            : "⋯"}
                        </Typography>
              ...
              {[0, 1, 2, 3].map((beat) => (
                <Box
                  ...
                  sx={{
                    ...
                    borderRadius: "50%",
                    backgroundColor:
                      currentBeat === beat ? "#2196f3" : "#e0e0e0",
