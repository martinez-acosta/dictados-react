...
export default function IntervalsTable() {
  // ---------- Tipos ----------
  type Dir = 'asc' | 'desc'
  type Kind = 'Justo' | 'Mayor' | 'Menor' | 'Aumentado' | 'Disminuido' | 'Tritono'

  // ---------- Colores ----------
  const getTypeColor = (type: Kind) => {
    switch (type) {
      case 'Justo':       return '#2e7d32'
      case 'Mayor':       return '#1565c0'
      case 'Menor':       return '#c62828'
      case 'Aumentado':   return '#6a1b9a'
      case 'Disminuido':  return '#ef6c00'
      case 'Tritono':     return '#8d6e63'
      default:            return '#666'
    }
  }
  ...
  const ACC_SYMBOL: Record<string,string> = { '': '', '#':'♯', '##':'𝄪', 'b':'♭', 'bb':'𝄫' }
  const shiftToAcc = (s:number) => (s===-2?'bb':s===-1?'b':s===1?'#':s===2?'##':'')
  ...
  const parseSPN = (spn: string): Parsed => {
    ...
    return { L, acc, shift: ACC_TO_SHIFT[acc as keyof typeof ACC_TO_SHIFT] ?? 0, oct }
  }
  const midiOf = ({ L, shift, oct }: { L:string, shift:number, oct:number }) =>
  ...
  const melissaIntervals = [
    { name: 'Unísono',   symbol: 'P1', semitones: 0,  type: 'Justo'     as Kind, steps: 0 },
    { name: '2ª menor',  symbol: 'm2', semitones: 1,  type: 'Menor'     as Kind, steps: 1 },
    { name: '2ª mayor',  symbol: 'M2', semitones: 2,  type: 'Mayor'     as Kind, steps: 1 },
    { name: '3ª menor',  symbol: 'm3', semitones: 3,  type: 'Menor'     as Kind, steps: 2 },
    { name: '3ª mayor',  symbol: 'M3', semitones: 4,  type: 'Mayor'     as Kind, steps: 2 },
    { name: '4ª justa',  symbol: 'P4', semitones: 5,  type: 'Justo'     as Kind, steps: 3 },
    { name: 'Tritono',   symbol: 'TT', semitones: 6,  type: 'Tritono'   as Kind, steps: 3 }, // A4 / d5
    { name: '5ª justa',  symbol: 'P5', semitones: 7,  type: 'Justo'     as Kind, steps: 4 },
    { name: '6ª menor',  symbol: 'm6', semitones: 8,  type: 'Menor'     as Kind, steps: 5 },
    { name: '6ª mayor',  symbol: 'M6', semitones: 9,  type: 'Mayor'     as Kind, steps: 5 },
    { name: '7ª menor',  symbol: 'm7', semitones: 10, type: 'Menor'     as Kind, steps: 6 },
    { name: '7ª mayor',  symbol: 'M7', semitones: 11, type: 'Mayor'     as Kind, steps: 6 },
    { name: 'Octava',    symbol: 'P8', semitones: 12, type: 'Justo'     as Kind, steps: 7 },
  ]
  ...
  const saidIntervals = [
    ...
    { name: '3ª mayor',       symbol: 'M3', semitones: 4,  type: 'Mayor'      as Kind, steps: 2 },
    { name: '4ª justa',       symbol: 'P4', semitones: 5,  type: 'Justo'      as Kind, steps: 3 },
    { name: '4ª aumentada',   symbol: 'A4', semitones: 6,  type: 'Aumentado'  as Kind, steps: 3 },
    { name: '5ª disminuida',  symbol: 'd5', semitones: 6,  type: 'Disminuido' as Kind, steps: 4 },
    { name: '5ª justa',       symbol: 'P5', semitones: 7,  type: 'Justo'      as Kind, steps: 4 },
    { name: '5ª aumentada',   symbol: 'A5', semitones: 8,  type: 'Aumentado'  as Kind, steps: 4 },
    { name: '6ª menor',       symbol: 'm6', semitones: 8,  type: 'Menor'      as Kind, steps: 5 },
    { name: '6ª mayor',       symbol: 'M6', semitones: 9,  type: 'Mayor'      as Kind, steps: 5 },
    { name: '7ª menor',       symbol: 'm7', semitones: 10, type: 'Menor'      as Kind, steps: 6 },
    { name: '7ª mayor',       symbol: 'M7', semitones: 11, type: 'Mayor'      as Kind, steps: 6 },
  ...
  // ---------- Estilos ----------
  const thBase: React.CSSProperties = { padding: '6px 8px', fontWeight: 700, fontSize: '0.85em', whiteSpace: 'nowrap' }
  const tdBase: React.CSSProperties = { padding: '6px 8px', fontSize: '0.85em', whiteSpace: 'nowrap' }
  const rootEs = (() => { const p = parseSPN(root); return spanishOf(p.L, p.acc, p.oct) })()
    ...
    <Paper sx={{ p: 2 }}>
          ...
          <FormControl size="small" sx={{ minWidth: 180 }}>
            ...
            >
              {NATURAL_ROOTS_4.map(n => {
                ...
                return <MenuItem key={n} value={n}>{es} — {n}</MenuItem>
              })}
            </Select>
          </FormControl>
      ...
      <Stack direction={{ xs:'column', lg:'row' }} spacing={3}>
        ...
        <Box sx={{ flex: 1, overflowX:'auto' }}>
          ...
          <table style={{ display:'table', borderCollapse:'collapse', width:'100%', tableLayout:'fixed' }} cellSpacing={0} cellPadding={0}>
            <colgroup>
              <col style={{ width: '15%' }} />
              <col style={{ width: '20%' }} />
              <col style={{ width: '45%' }} />
            ...
            <thead>
              <tr style={{ borderBottom: '1px solid #ddd' }}>
                ...
                <th style={{ ...thBase, textAlign:'left'  }}>Símbolo / Intervalo</th>
                <th style={{ ...thBase, textAlign:'left'  }}>Destino</th>
              </tr>
            </thead>
            <tbody>
              {melissaIntervals.map((iv, i) => {
                const target = spellIntervalTarget(root, iv.semitones, iv.steps, direction)
                // Destacar si es el intervalo seleccionado O si es tritono y se seleccionó A4/d5
                const isHighlighted = selectedInterval === iv.symbol ||
                  (iv.symbol === 'TT' && (selectedInterval === 'A4' || selectedInterval === 'd5'))
                const backgroundColor = isHighlighted
                  ? 'rgba(25, 118, 210, 0.15)'
                  : (i%2===0 ? '#fafafa' : '#fff')
                return (
                  <tr key={`mel-${iv.symbol}`} style={{ borderBottom:'1px solid #eee', background: backgroundColor }}>
                    <td style={{ ...tdBase, textAlign:'center' }}>{iv.semitones / 2}</td>
                    <td style={{ ...tdBase, textAlign:'center', fontWeight:700, color:getTypeColor(iv.type), fontSize:'0.65em' }}>{iv.type}</td>
                    <td style={{ ...tdBase, fontFamily:'monospace', fontWeight:700 }}>
                      {iv.symbol} <span style={{ opacity:0.6 }}>/</span> <span style={{ fontWeight:500, fontFamily:'inherit' }}>{iv.name}</span>
                    </td>
                    <td style={{ ...tdBase, fontFamily:'monospace' }}>{target.es}</td>
                  </tr>
                )
              })}
            </tbody>
          ...
          <table style={{ display:'table', borderCollapse:'collapse', width:'100%', tableLayout:'fixed' }} cellSpacing={0} cellPadding={0}>
            ...
            <tbody>
              {saidIntervals.map((iv, i) => {
                ...
                const isHighlighted = selectedInterval === iv.symbol ||
                  ((iv.symbol === 'A4' || iv.symbol === 'd5') && selectedInterval === 'TT')
                const backgroundColor = isHighlighted
                  ? 'rgba(25, 118, 210, 0.15)'
                  : (i%2===0 ? '#fafafa' : '#fff')
                return (
                  <tr key={`said-${iv.symbol}`} style={{ borderBottom:'1px solid #eee', background: backgroundColor }}>
                    <td style={{ ...tdBase, textAlign:'center' }}>{iv.semitones / 2}</td>
                    <td style={{ ...tdBase, textAlign:'center', fontWeight:700, color:getTypeColor(iv.type), fontSize:'0.65em' }}>{iv.type}</td>
                    <td style={{ ...tdBase, fontFamily:'monospace', fontWeight:700 }}>
                      {iv.symbol} <span style={{ opacity:0.6 }}>/</span> <span style={{ fontWeight:500, fontFamily:'inherit' }}>{iv.name}</span>
      ...
      <Stack spacing={2} sx={{ mt: 3 }}>
        ...
        <Box sx={{ p: 2, bgcolor: 'rgba(76, 175, 80, 0.08)', borderRadius: 1 }}>
          ...
          {(() => {
            ...
            const scale: Array<{note: string, midi: number, interval: string}> = []
            ...
            for (let i = 0; i < pattern.length; i++) {
              ...
              // Calcular octava
              const nextOct = letterIndex === 6 ? currentNote.oct + 1 : currentNote.oct
              const naturalMidi = 12 * (nextOct + 1) + NAT_PC[nextLetter]
              const delta = targetMidi - naturalMidi

              const acc = delta === 0 ? '' : delta === 1 ? '#' : delta === -1 ? 'b' : ''
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
                {scale.map((item, index) => (
                  <Box key={index} sx={{ display: 'flex', alignItems: 'center' }}>
                    <Box sx={{
                      px: 1, py: 0.3,
                      bgcolor: index === 0 ? '#2e7d32' : 'rgba(76, 175, 80, 0.15)',
                    ...
                    {index < scale.length - 1 && (
                      <Box sx={{
                        ...
                        bgcolor: scale[index + 1].interval === 'T' ? '#1976d2' : '#f57c00',
                        color: 'white',
                        borderRadius: '50%',
                        fontSize: '0.7em',
                        fontWeight: 700
          ...
          <Typography variant="caption" sx={{ mt: 1, display: 'block', color: '#2e7d32' }}>
            <strong style={{color: '#1976d2'}}>T</strong> = Tono (2 semitonos) • <strong style={{color: '#f57c00'}}>S</strong> = Semitono (1 semitono) • Patrón universal de la escala mayor
          </Typography>
        </Box>
      ...
      <Typography variant="caption" sx={{ mt: 1.5, display: 'block', color: 'text.secondary', fontStyle: 'italic' }}>
        "Tritono" (3 tonos) se puede escribir como 4ª aumentada (A4) o 5ª disminuida (d5). Ambas suenan igual, pero su función y escritura cambian.
      </Typography>
    </Paper>
  )
}