...
function generateMeasureWithPitches(
  ...
  while (sum < target - 1e-6) {
    ...
    if (
      ...
      pick =
        Math.random() < 0.7 ? pickRandom(candNotes) : pickRandom(candRests);
    }
  ...
  const finalSum = seq.reduce(
    (a, n) => a + (DUR_TO_PULSES[n.duration] || 0),
    0,
  );
  if (Math.abs(finalSum - target) > 1e-6) {
    const remaining = target - finalSum;
    while (
      seq.reduce((a, n) => a + (DUR_TO_PULSES[n.duration] || 0), 0) <
      target - 1e-6
    ) {
...
function equalMeasures(a: NoteWithPitch[], b: NoteWithPitch[]) {
  ...
  for (let i = 0; i < a.length; i++) {
    if (a[i].duration !== b[i].duration || a[i].pitch !== b[i].pitch)
      return false;
  }
  return true;
}
...
export default function RitmicaConAlturas() {
  ...
  const pitchPool = useMemo(
    () => NOTE_RANGES[noteRange] || NOTE_RANGES["C4-C5"],
    [noteRange],
  );
  ...
  function nuevaSecuencia() {
    const sol = Array.from({ length: barsCount }, () =>
      generateMeasureWithPitches(includeRests, pitchPool),
    );
    setSolution(sol);
    ...
    <Box sx={{ width: "100%", px: 2, pb: 4 }}>
      ...
      <Paper sx={{ p: 2 }}>
        ...
        >
          ...
          >
            ...
            <FormControl size="small" sx={{ width: 120 }}>
              ...
              <Select
                value={mode}
                onChange={(e) => setMode(e.target.value as "train" | "exam")}
                label="Modo"
              >
        ...
        {mode === "exam" && (
          <Stack spacing={1} sx={{ mb: 2 }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>
              Figuras disponibles: redonda, blanca, negra y{" "}
              <strong>doble corchea</strong>
          ...
          {(mode === "exam" ? answers : solution).map((m, idx) => {
            ...
            return (
              <Paper
                key={`bar-${idx}`}
                variant="outlined"
                onClick={() => mode === "exam" && setActiveBar(idx)}
                sx={{
                  flex: "1 1 auto",
                  minWidth: "350px",
                  p: 1.5,
                  cursor: mode === "exam" ? "pointer" : "default",
                  borderColor: isActive ? "primary.main" : "divider",
                  borderWidth: isActive ? 2 : 1,
                  bgcolor: isActive ? "rgba(25,118,210,.03)" : undefined,
                }}
              >
                <Stack
                  direction="row"
                ...
                >
                  <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    {mode === "exam" && graded.length > 0 && graded[idx] && (
                      <Chip
                        size="small"
                        label={`${graded[idx].filter(Boolean).length}/${graded[idx].length}`}
                        color={
                          graded[idx].every(Boolean) ? "success" : "warning"
                        }
                        variant="outlined"
                      />
                    )}
                      ...
                      .map((n, noteIdx) => {
                        ...
                        return (
                          <Grid
                            ...
                            xs={6}
                            md={4}
                            key={`input-${idx}-${noteIdx}`}
                          >
                            <TextField
                              fullWidth
                              size="small"
                              label={`Nota ${noteIdx + 1}`}
                              value={input}
                              onChange={(e) =>
                                updateNotePitch(idx, noteIdx, e.target.value)
                              }
                              error={Boolean(input && !parsed.valid)}
                              helperText={
                                input && !parsed.valid
                                  ? "Ej: C4 o Do4"
                                  : NOTE_NAMES[n.pitch] || n.pitch
                              }
                            />
                          </Grid>
                        );
        ...
        >
          💡{" "}
          {mode === "train"
            ? "Modo entrenamiento: escucha y observa la solución."
            : "Modo examen: construye el ritmo con la paleta y escribe la altura de cada nota (ej: C4 o Do4)."}
        </Typography>
      </Paper>
    </Box>
  );
