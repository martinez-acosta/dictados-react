  ...
  useEffect(() => {
    ...
    try {
      const vs = notes.map((d, i) => {
        const note = new StaveNote({ clef: 'treble', keys: ['b/4'], duration: d })
      ...
      try {
        const all8 = notes.map((d, i) => ({ d, i })).filter(x => x.d === '8').map(x => x.i)
        for (let k = 0; k < all8.length; k += 2) {
          const i1 = all8[k], i2 = all8[k+1]
...
function totalPulses(ds: string[]) { return ds.reduce((a,d)=>a+(DUR_TO_PULSES[d]||0),0) }
...
export default function RitmicaTablaYEntrenador() {
  ...
  function nuevaSecuencia() {
    const sol = Array.from({length:barsCount}, () => generateMeasureBasic4(includeRests))
    setSolution(sol)
  ...
  function addToActive(durs: string[] | string) {
    ...
    setAnswers(prev => {
      ...
      const sum = current.notes.reduce((a,d)=>a+(DUR_TO_PULSES[d]||0),0)
  ...
  const allComplete = useMemo(() =>
    answers.every(m => Math.abs(m.notes.reduce((a,d)=>a+(DUR_TO_PULSES[d]||0),0) - target) < 1e-6),
    [answers]
  )
    ...
    <Box sx={{ width:'100%', px:2, pb:4 }}>
      ...
      <Paper sx={{ p:2 }}>
            ...
            return (
              <Paper
                ...
                sx={{
                  ...
                  bgcolor: activeBar === idx ? 'rgba(25,118,210,.03)' : undefined,
                  '&:hover': { bgcolor: activeBar === idx ? 'rgba(25,118,210,.05)' : 'rgba(0,0,0,.02)' }
                }}
              >
                <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Typography sx={{ fontWeight: 700, color: activeBar === idx ? 'primary.main' : 'text.primary' }}>
                      Compás {idx + 1}
                    </Typography>
                    ...
                    />
                    {graded.length > 0 && (
                      <Chip size="small" label={correct ? 'Correcto ✓' : 'Revisa'}
                            color={correct ? 'success' : 'warning'} variant="outlined" />
                    )}
                  </Stack>

                  <Stack direction="row" spacing={0.5} alignItems="center">
                    <IconButton size="small"
                      onClick={(e) => { e.stopPropagation(); undoActive() }}
                      disabled={activeBar !== idx || m.notes.length === 0}
                      sx={{ opacity: (activeBar === idx && m.notes.length > 0) ? 1 : 0.3 }}>
                      <Undo fontSize="small" />
                    </IconButton>
                    <IconButton size="small"
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center', mb: showSolution ? 1 : 0 }}>
                  <VFMeasure
        ...
        <Stack direction="row" spacing={2} justifyContent="flex-end" sx={{ mt:2 }}>
          <Button variant="outlined" onClick={()=>setAnswers(Array.from({length:barsCount},()=>({notes:[]})) )}>
            Limpiar todo
          </Button>
          <Button variant="contained" onClick={calificar} disabled={!allComplete}>
            Calificar
          </Button>
        </Stack>
      </Paper>
