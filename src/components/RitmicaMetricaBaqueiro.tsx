...
type Duration = "w" | "h" | "q" | "8" | "wr" | "hr" | "qr" | "8r";
type Exercise = {
  id: number;
  title: string;
  description: string;
...
function generateExerciseVariant(exerciseId: number, count: number): Duration[][] {
...
function RhythmSystemRow({
    ...
    measures.forEach((measure, localIndex) => {
      ...
      if (showNotes) {
        ...
        try {
          ...
          const formatWidth = globalIndex === 0 ? staveWidth - 92 : staveWidth - 16;
  ...
  return (
    <Paper variant="outlined" sx={{ p: 1, minWidth: canvasWidth + 16 }}>
      <Box sx={{ position: "relative", width: canvasWidth, height: 122, mx: "auto" }}>
        <div ref={hostRef} />
          ...
          return (
            <Box
              ...
              sx={{
                ...
                cursor: "pointer",
                borderRadius: 1,
                border: highlighted ? "2px solid #ff6b35" : "2px solid transparent",
                backgroundColor: highlighted ? "rgba(255,107,53,.08)" : "transparent",
...
export default function RitmicaMetricaBaqueiro() {
  ...
  const [generatedMeasures, setGeneratedMeasures] = useState<Duration[][] | null>(
    null,
  );
  ...
  const rows = useMemo(() => chunkMeasures(displayMeasures, 5), [displayMeasures]);
  ...
  const playSequence = useCallback(
      ...
      for (let rep = 0; rep < loops; rep++) {
        for (const measureIndex of indexes) {
          ...
          measure.forEach((dur) => {
            ...
            if (!dur.endsWith("r")) {
              const timer = window.setTimeout(() => {
                noteRef?.triggerAttackRelease("C4", Math.max(0.06, durSecs * 0.88));
    ...
    [bpm, clearPlayback, displayMeasures],
  );
  ...
  }, [isPlaying, questionInfinite, questionMode, showAnswer, startQuestionRound]);
  ...
  return (
    <Box sx={{ width: "100%", px: 2, pb: 4 }}>
      ...
      <Paper sx={{ p: 2.25, mb: 2 }}>
        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap alignItems="center">
          ...
          <TextField
            ...
            label="Repeticiones"
            value={repeats}
            onChange={(e) => setRepeats(Math.max(1, Math.min(8, Number(e.target.value) || 1)))}
          ...
          <Button
            ...
            startIcon={<PlayArrow />}
            disabled={isPlaying}
            onClick={() => void playSequence([...Array(measureCount).keys()], repeats)}
          >
            Escuchar ejercicio
          </Button>
          ...
          <Button
            ...
            onClick={() => {
              ...
              setShowAnswer(true);
              setCurrentMeasure(0);
              setGeneratedMeasures(generateExerciseVariant(exerciseId, measureCount));
            }}
          >
            Nuevo ejercicio
          </Button>
          ...
          <Button
            ...
            onClick={() => setGeneratedMeasures(null)}
          >
            Volver al libro
          </Button>
        </Stack>

        <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap alignItems="center" sx={{ mt: 1.25 }}>
          <Button
            variant="outlined"
          ...
          <Button
            ...
            onClick={() => setCurrentMeasure((v) => Math.min(measureCount - 1, v + 1))}
          >
            Compás siguiente
          </Button>
          <Chip label={`Compás seleccionado: ${currentMeasure + 1}`} />
          <Chip icon={<RestartAlt />} label={`${measureCount} compases`} variant="outlined" />
          <Chip label={`${bpm} BPM`} variant="outlined" />
          <Chip
            color={generatedMeasures ? "success" : "default"}
            label={generatedMeasures ? "Variación generada" : "Versión del libro"}
            variant={generatedMeasures ? "filled" : "outlined"}
          />
        </Stack>
      ...
      {isPlaying && (
        <Paper sx={{ p: 1.25, mb: 2, backgroundColor: "rgba(33, 150, 243, .08)" }}>
      ...
      {questionMode && (
        <Paper sx={{ p: 1.25, mt: 2, border: "1px dashed #ff9800", background: "rgba(255,152,0,.08)" }}>
          <Typography variant="body2" sx={{ fontWeight: 700 }}>
            Modo pregunta activo
          </Typography>
          <Typography variant="body2" sx={{ color: "text.secondary" }}>
            Notación y resaltado naranja ocultos hasta mostrar respuesta.
            {questionMeasure != null ? ` Último compás preguntado: ${questionMeasure + 1}.` : ""}
          </Typography>
        </Paper>
      )}
    </Box>
