...
export default function IntervalosDo4Hardcoded() {
  ...
  const [currentPlayingSymbol, setCurrentPlayingSymbol] = useState<
    string | null
  >(null);
  const [currentNoteDisplay, setCurrentNoteDisplay] = useState<NoteDisplay>({
    spn: "",
    solfege: "",
    show: false,
  });
  const [status, setStatus] = useState(
    'Selecciona un intervalo y presiona "Nuevo".',
  );

  const [currentInterval, setCurrentInterval] =
    useState<CurrentInterval | null>(null);
  ...
  function drawStaff(): void {
    ...
    vfA.current = new Factory({
      renderer: { elementId: wrapA.current.id, width: 10, height: 10 },
    });
    ...
    if (isCompleteMode) {
      ...
      for (let interval of INTERVALS) {
        const targetKey = calculateIntervalFromBase(
          baseNote,
          interval.semitones,
          direction,
        );
      ...
      if (!currentInterval) {
        const containerWidth =
          wrapA.current.parentElement?.offsetWidth || window.innerWidth - 100;
  ...
  function generateInterval(
    ...
    const target = calculateIntervalFromBase(
      baseNote,
      row.semitones,
      direction,
    );
    ...
    setStatus(
      `Intervalo: ${row.name} ${direction === "ascending" ? "ascendente" : "descendente"} desde ${labelSPN(base)}`,
    );
  }
  ...
  function generateCompleteIntervals(): void {
    ...
    setStatus(
      `🎯 Modo Completo - Todos los intervalos desde ${BASE_NOTES.find((n) => n.step === baseNote)?.label}4`,
    );
  }
  ...
  async function playCompleteSequence(): Promise<void> {
    ...
    const playChordComplete = (
      ...
      const tId = window.setTimeout(() => {
        ...
        const solfegeLabels = notes
          .map((vfKey) => labelSolfege(vfKey))
          .join(" + ");
        setCurrentNoteDisplay({
          spn: noteLabels,
          solfege: solfegeLabels,
          show: true,
        });
  ...
  async function playInterval(): Promise<void> {
    if (!isCompleteMode && !currentInterval) {
      setStatus("Primero genera un intervalo.");
      return;
    }
    if (isPlaying) {
      setIsPlaying(false);
  ...
  function checkAnswer(userSymbol: string): void {
    ...
    setStatus(
      ok
        ? "¡Correcto!"
        : `Incorrecto. Era ${currentInterval.name} ${direction === "ascending" ? "ascendente" : "descendente"}.`,
    );
  }
      ...
      <Stack spacing={2}>
        ...
        <Paper sx={{ p: 2 }}>
          ...
          >
            ...
            >
                  ...
                  {INTERVALS.map((row, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: "2px 4px", fontSize: "0.78em" }}>
                          Do4
                        </td>
                        <td
                          style={{
                        ...
                        >
                          {row.symbol} <span style={{ opacity: 0.6 }}>/</span>{" "}
                          <span
              ...
              >
                Base fija: Do4 (C4). Dirección asc/desc cambia la nota destino y
            ...
            {/* Controles */}
            <Box
              sx={{ width: { xs: "100%", md: 360 }, flex: "0 0 auto", ml: 2 }}
            >
              <Typography
                variant="h6"
              ...
              <Grid container spacing={1.2}>
                <Grid item xs={6}>
                  <FormControl fullWidth size="small">
                    ...
                    >
                      {INTERVALS.map((r) => (
                        <MenuItem key={r.symbol} value={r.symbol}>
                          {r.name}
                        </MenuItem>
                      ))}
                    </Select>
                ...
                <Grid item xs={4}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      value={dur}
                      onChange={(e) =>
                        setDur(e.target.value as "q" | "8" | "h")
                      }
                    >
                ...
                <Grid item xs={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      value={mode}
                      onChange={(e) =>
                        setMode(e.target.value as "train" | "exam")
                      }
                    >
                ...
                <Grid item xs={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      ...
                      value={engine}
                      onChange={(e) =>
                        setEngine(e.target.value as "yamaha" | "synth")
                      }
                    >
                ...
                <Grid item xs={6}>
                  <FormControl fullWidth size="small">
                    ...
                    <Select
                      labelId="base-note-label"
                      label="Nota Base"
                      value={baseNote}
                      onChange={(e) => setBaseNote(e.target.value as string)}
                      disabled={isCompleteMode}
                    >
                      {BASE_NOTES.map((note) => (
                        <MenuItem key={note.step} value={note.step}>
                          {note.label} ({note.spn}4)
                        </MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                ...
                <Grid item xs={6}>
                  <TextField
                    ...
                    onChange={(e) =>
                      setPauseBetweenIntervals(Number(e.target.value))
                    }
        ...
        <Paper sx={{ p: 2 }}>
          ...
          {mode === "train" && (isCompleteMode || currentInterval) && (
            <Box
              sx={{
                ...
                alignItems: "center",
                py: 1,
                backgroundColor: isCompleteMode
                  ? "rgba(76, 175, 80, 0.1)"
                  : "rgba(33, 150, 243, 0.1)",
                borderRadius: 1,
                mb: 1,
            ...
            >
              <Typography
                ...
                sx={{
                  fontWeight: "bold",
                  color: isCompleteMode ? "#4caf50" : "#1976d2",
                  textAlign: "center",
                }}
              >
                {isCompleteMode ? (
                  <>
                    🎯 Modo Completo — Todos los intervalos desde{" "}
                    {BASE_NOTES.find((n) => n.step === baseNote)?.label}4{" "}
                    {direction === "ascending" ? "↗️" : "↘️"}
                    {isPlaying && (
                      <Typography
                        component="span"
                        sx={{ ml: 2, fontSize: "0.8em", opacity: 0.8 }}
                      >
                  ...
                  <>
                    ...
                    {currentInterval!.direction === "ascending" ? "↗️" : "↘️"} —{" "}
                    {labelSPN(currentInterval!.base)} →{" "}
                    {labelSPN(currentInterval!.target)} × {repetitions}
                  </>
                )}
              </Typography>
            </Box>
          ...
          {mode === "train" && currentInterval && (
            ...
            >
              <Chip
                label={`${currentInterval.semitones} semitonos`}
                size="small"
                color="primary"
              />
              <Chip
            ...
            <Grid container spacing={1.5}>
              ...
              <Grid item xs={12} md={4}>
                <Button
                  ...
                  onClick={() => checkAnswer(selectedSymbol)}
                >
        ...
        <Box
          className="status"
          sx={{ p: 2, backgroundColor: "rgba(0,0,0,0.05)", borderRadius: 1 }}
        >
          {status}
        </Box>
      </Stack>
