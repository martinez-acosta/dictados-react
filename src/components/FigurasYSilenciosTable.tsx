...
function VFPairCell({ note, rest }: { note: string; rest: string }) {
  ...
  useEffect(() => {
    ...
    try {
      const vf = new Factory({
        renderer: { elementId: idRef.current, width: MAIN_W, height: MAIN_H },
      });
      const ctx = vf.getContext();
      ctx.scale(0.5, 0.5);
      const stave = new Stave(STAVE_X, STAVE_Y, MAIN_STAVE_W);
      stave.addClef("treble").setContext(ctx).draw();
      const n1 = new StaveNote({
        clef: "treble",
        keys: ["b/4"],
        duration: note,
      });
  ...
  return (
    <div
      ref={hostRef}
      style={{ width: MAIN_W, height: MAIN_H, background: BG }}
    />
  );
}
...
function buildVariant(id: string): VariantSpec | null {
  switch (id) {
    case "w":
      return { notes: ["h", "h"], tie: [0, 1], stemDir: 1 }; // redonda = 2 blancas ligadas
    case "h":
      return { notes: ["q", "q"], tie: [0, 1], stemDir: 1 }; // blanca = 2 negras ligadas
    case "q":
      return { notes: ["8", "8"], beamed: true, stemDir: 1 }; // arriba
    case "8":
      return { notes: ["16", "16"], beamed: true, stemDir: -1 }; // abajo
    case "16":
      return { notes: ["32", "32"], beamed: true, stemDir: 1 }; // arriba
    case "32":
      return { notes: ["64", "64"], beamed: true, stemDir: -1 }; // abajo
    case "64":
      return { notes: ["128", "128"], beamed: true, stemDir: 1 }; // arriba
    default:
      return null;
  }
}
...
function VFVariantCell({ forId }: { forId: string }) {
  ...
  useEffect(() => {
    ...
    try {
      const vf = new Factory({
        renderer: { elementId: idRef.current, width: VAR_W, height: VAR_H },
      });
      const ctx = vf.getContext();
      ctx.scale(0.5, 0.5);
      const stave = new Stave(VAR_STAVE_X, VAR_STAVE_Y, VAR_STAVE_W).addClef(
        "treble",
      );
  ...
  return (
    <div
      ref={hostRef}
      style={{ width: VAR_W, height: VAR_H, background: BG }}
    />
  );
}
    ...
    >
      <Grid container spacing={1} alignItems="center" wrap="nowrap">
        <Grid item xs zeroMinWidth>
          <Typography sx={{ fontWeight: 600, fontSize: 13.5, lineHeight: 1.2 }}>
            {row.figura}
          </Typography>
        </Grid>
        <Grid item xs>
          <Box sx={{ width: MAIN_W, mx: "auto" }}>
            <VFPairCell note={row.durNota} rest={row.durSilencio} />
          </Box>
        </Grid>
        <Grid item xs>
          <Box sx={{ width: VAR_W, mx: "auto" }}>
            <VFVariantCell forId={row.id} />
          </Box>
        </Grid>
        <Grid item sx={{ width: VALUE_COL_WIDTH, textAlign: "center" }}>
          <Typography
...
function TableBlock({ rows }: { rows: FiguraRow[] }) {
  return (
    <Box>
      ...
      >
        {rows.map((row, i) => (
          <Row4 key={row.id} row={row} first={i === 0} />
        ))}
      </Box>
    </Box>
  );
...
function splitInto<T>(arr: T[], columns: number): T[][] {
  ...
  for (let i = 0; i < columns; i++)
    out.push(arr.slice(i * perCol, (i + 1) * perCol));
  return out;
}

export default function FigurasYSilenciosTable({
  showHeader = true,
}: FigurasYSilenciosTableProps) {
    ...
    <Paper sx={{ p: 2, maxWidth: 1800, mx: "auto", bgcolor: BG }}>
      {showHeader && (
        ...
        >
          ...
          <Chip
            label="Notación: primero figura, luego silencio"
            size="small"
            variant="outlined"
          />
          <Chip
            label="Variante con barra / ligadura"
            size="small"
            variant="outlined"
            color="secondary"
          />
      ...
      >
        {columns.map((colRows, idx) => (
          <TableBlock key={idx} rows={colRows} />
        ))}
      </Box>
    </Paper>
  );
