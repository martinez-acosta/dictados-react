...
function parseSpelled(n: string): {
...
const LETTER_TO_SOLFEGE: Record<
  "C" | "D" | "E" | "F" | "G" | "A" | "B",
  string
...
function solfegeFromSpelled(name: string): string {
  const { letter, acc } = parseSpelled(name);
  const base =
    LETTER_TO_SOLFEGE[letter as keyof typeof LETTER_TO_SOLFEGE] || "Do";
  if (!acc) return base;
  if (acc === "#") return `${base}#`;
  if (acc === "##") return `${base}##`;
...
const HARDCODED: Record<
  Quality,
  Array<{ root: string; spelled: [string, string, string] }>
> = {
  major: [
