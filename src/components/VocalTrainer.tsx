...
function midiToToneName(m: number) {
  const name = NAMES_SHARP[m % 12];
  const o = Math.floor(m / 12) - 1;
  return `${name}${o}`;
}
function keyFromMidi(m: number) {
  const name = NAMES_SHARP_LOWER[m % 12];
  const o = Math.floor(m / 12) - 1;
  return `${name}/${o}`;
}
function toneNoteFromKey(key: string) {
  return midiToToneName(midiFromKey(key));
}
function prettyFromKey(key: string) {
  const [s, o] = key.split("/");
  const sym =
    (LETTERS as any)[s] ??
    s.toUpperCase().replace(/#/g, "♯").replace(/b/g, "♭");
  return `${sym}${o}`;
}
function labelSolfege(key: string) {
  const [s, o] = key.split("/");
  return `${(SOLFEGE as any)[s] || s.toUpperCase()}${o}`;
}
...
function pianoLike(
  ...
  [
    ...
    { t: "sine" as OscillatorType, mult: 4, g: 0.1 },
  ].forEach((p) => {
    ...
    if (activeOsc) {
      activeOsc.add(o);
      o.addEventListener("ended", () => activeOsc.delete(o));
    }
...
function sineGuide(
  ...
  if (glideToFreq != null) {
    o.frequency.linearRampToValueAtTime(glideToFreq, time + durSec);
  }
  o.connect(g);
  g.connect(master!);
...
function metronomeClickNow(volDb = -3, activeOsc?: Set<OscillatorNode>) {
  ...
  if (activeOsc) {
    activeOsc.add(osc);
    osc.addEventListener("ended", () => activeOsc.delete(osc));
  }
...
const RANGE_LABEL: Record<VoiceRange, string> = {
  bajo: "Bajo (≈E2–E4)",
  baritono: "Barítono (≈A2–A4)",
  tenor: "Tenor (≈C3–C5)",
  contralto: "Contralto (≈F3–F5)",
  mezzo: "Mezzosoprano (≈A3–A5)",
...
const MAJOR_DEGREE_TO_ST: Record<number, number> = {
  1: 0,
  2: 2,
  3: 4,
  4: 5,
...
const EXERCISES: Exercise[] = [
  {
    id: "scale_up_down",
    name: "Escala 1–8–1 (Mayor)",
    kind: "pattern",
    patternDegrees: [1, 2, 3, 4, 5, 6, 7, 8, 8, 7, 6, 5, 4, 3, 2, 1],
  },
  {
    id: "five_tone",
    name: "5 tonos 1–5–1",
    kind: "pattern",
    patternSemitones: [0, 2, 4, 5, 7, 5, 4, 2, 0],
  },
  {
    id: "arpeggio_ma",
    name: "Arpegio 1–3–5–8–5–3–1",
    kind: "pattern",
    patternSemitones: [0, 4, 7, 12, 7, 4, 0],
  },
  {
    id: "octave_clean",
    name: "Octava limpia 1–8–1",
    kind: "pattern",
    patternSemitones: [0, 12, 0],
  },
  {
    id: "fa_si",
    name: "Fa–Si (gliss tritono)",
    kind: "gliss",
    steps: 24,
    spanSemitones: 6,
  },
...
const EXERCISE_HELP: Record<string, string> = {
  scale_up_down:
    "Coordina soporte y paso a mixto en la octava. Útil para estabilizar el 2º pasaggio sin forzar.",
  five_tone:
    "Afinación y homogeneidad en registro medio. Ideal para calentar y nivelar vocales.",
  arpeggio_ma:
    "Activa resonancias y el registro mixto. Trabaja saltos limpios sin empujar el pecho.",
  octave_clean:
    "Practica el salto de octava limpio (1–8–1). Clave para el control del ataque en el agudo.",
  fa_si:
    "Glissando de tritono (4ª aumentada, 6 semitonos): desliza suavemente hacia arriba y de regreso. Desarrolla control en intervalos amplios y tensión armónica sin cortes.",
  sirena:
    "Desliza (gliss) a través de los pasaggi sin cortes. Afloja tensiones y mejora continuidad.",
  sirena_personal:
    "Sirena personalizada C3→C#5 (25 semitonos). Abarca todo tu rango vocal con glissando suave.",
  sustain:
    "Control de aire y dinámica (crescendo–diminuendo) manteniendo afinación estable.",
};
...
export default function VocalTrainer() {
  ...
  function ensureAudio() {
    ...
    try {
      (Tone as any).Destination.mute = false;
    } catch {}
    if (master && ctx) {
      try {
  ...
  function stopAllSounds(fromUnmount = false) {
    clearAllTimeouts();
    try {
      (toneSamplerRef as any)?.releaseAll?.();
    } catch {}
  ...
  useEffect(() => {
    ...
    let offsets: number[] = [];
    if (ex.kind === "pattern") {
      if (ex.patternDegrees)
        offsets = ex.patternDegrees.map(
          (d) => MAJOR_DEGREE_TO_ST[d as 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8],
        );
      ...
      if (ex.id === "sirena_personal") {
        ...
        const up = Array.from({ length: steps }, (_, i) =>
          Math.round((totalSpan * i) / (steps - 1)),
        );
        const down = Array.from(
          { length: steps },
          (_, i) => totalSpan - Math.round((totalSpan * i) / (steps - 1)),
        );
        offsets = [...up, ...down];
        rootMidi = c3Midi; // Forzar C3 como base
      } else {
        const steps = ex.steps ?? 36,
          span = ex.spanSemitones ?? 12;
        const up = Array.from({ length: steps }, (_, i) =>
          Math.round((span * i) / (steps - 1)),
        );
        const down = Array.from(
          { length: steps },
          (_, i) => span - Math.round((span * i) / (steps - 1)),
        );
        offsets = [...up, ...down];
      }
    ...
    if (autoFit && ex.id !== "sirena_personal") {
      ...
      for (const tp of tryFits) {
        ...
        const inside =
          Math.max(0, limits.min - low) + Math.max(0, high - limits.max);
    ...
    setCurrentScale({
      name: `${SCALE_META[selectedScale]} — ${EXERCISES.find((e) => e.id === exerciseId)!.name} — ${RANGE_LABEL[range]}`,
      notes: keys,
    });
  ...
  useEffect(() => {
    drawStaff();
  }, [currentScale, currentPlayingNote, dur]);
  ...
  function drawStaff() {
    ...
    const optimalNoteWidth = Math.min(
      maxNoteWidth,
      Math.max(
        minNoteWidth,
        (availableWidth - staveMargins - LEFT_PAD - RIGHT_PAD) / notes.length,
      ),
    );
    const calculatedWidth = Math.min(
      availableWidth,
      staveMargins + notes.length * optimalNoteWidth + LEFT_PAD + RIGHT_PAD,
    );

    vfA.current = new Factory({
      renderer: {
        elementId: wrapA.current.id,
  ...
  async function play() {
    ...
    if (countIn) {
      const id1 = window.setTimeout(
        () => metronomeClickNow(-3, activeOscillatorsRef.current),
        startDelayMs - 2 * beatSec * 1000,
      );
      const id2 = window.setTimeout(
        () => metronomeClickNow(-3, activeOscillatorsRef.current),
        startDelayMs - 1 * beatSec * 1000,
      );
    ...
    const schedulePhraseOnce = (phraseStartMs: number) => {
      currentScale.notes.forEach((key, i) => {
        ...
        // UI
        const tIdx = window.setTimeout(
          () => setCurrentPlayingNote(i),
          whenMs - performance.now(),
        );
        const tLbl = window.setTimeout(
          () =>
            setCurrentNoteDisplay({
              spn: prettyFromKey(key),
              solfege: labelSolfege(key),
              show: true,
            }),
          whenMs - performance.now(),
        );
        ...
        const tAud = window.setTimeout(() => {
          ...
          if (guideMode === "piano") {
            if (toneSamplerRef) {
              toneSamplerRef.triggerAttackRelease(
                toneNoteFromKey(key),
                noteDurSec * 0.95,
              );
            } else {
              pianoLike(
                f,
                (ctx as AudioContext).currentTime,
                noteDurSec * 0.95,
            ...
            const next =
              i < currentScale.notes.length - 1
                ? currentScale.notes[i + 1]
                : undefined;
            const glide =
              (ex.kind === "gliss" || ex.gliss) && next
                ? 440 * Math.pow(2, (midiFromKey(next) - 69) / 12)
                : undefined;
            sineGuide(
              f,
              (ctx as AudioContext).currentTime,
      ...
      <Stack spacing={2}>
        ...
        <Paper sx={{ p: 2 }}>
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} sm={6} md={3}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  label="Perfil / Rango"
                  value={range}
                  onChange={(e) => setRange(e.target.value as VoiceRange)}
                >
                  {(
                    [
                      "bajo",
            ...
            <Grid item xs={6} sm={3} md={1.5}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  label="Duración"
                  value={dur}
                  onChange={(e) => setDur(e.target.value as any)}
                >
            ...
            <Grid item xs={6} sm={3} md={1.5}>
              <TextField
                fullWidth
                size="small"
                label="Descanso (beats)"
                type="number"
                inputProps={{ min: 0, max: 16 }}
                value={restBeats}
                onChange={(e) => setRestBeats(Number(e.target.value))}
              />
            </Grid>
          ...
          >
            <Typography
              variant="subtitle2"
              sx={{ fontWeight: 700, color: "#1976d2", mb: 0.25 }}
            >
            ...
            <Box sx={{ mt: 1, display: "flex", gap: 1, flexWrap: "wrap" }}>
              {fitInfo.transposition !== 0 && (
                <Chip
                  size="small"
                  color="info"
                  label={`Transpuesto ${fitInfo.transposition > 0 ? "+" : ""}${fitInfo.transposition} st para encajar en ${RANGE_LABEL[range]}`}
                />
              )}
              <Chip
                size="small"
              ...
              {showWarning && (
                <Chip
                  size="small"
                  color="warning"
                  label="⚠ En zona de pasaggio — sube con soporte y poca presión"
                />
              )}
            </Box>
          )}
        ...
        <Paper sx={{ p: 2 }}>
          ...
          >
            Objetivo de precisión: ±10 cents. Si te alejas, baja el tono raíz o
            usa la guía "Sine".
          </Typography>
        </Paper>
          ...
          {currentScale && (
            ...
            >
              {currentScale.notes.map((n, idx) => (
                <Chip
                  key={`${n}-${idx}`}
                  label={prettyFromKey(n)}
                  size="small"
                  color={currentPlayingNote === idx ? "primary" : "default"}
                />
              ))}
            </Box>
          )}
